...
class QueryParser {
    ...
    private String consumeSubQuery() {
        ...
        while (!this.tq.isEmpty()) {
            if (this.tq.matches("(")) {
                ...
                sb.append("[");
                sb.append(this.tq.chompBalanced(JsonLexerKt.BEGIN_LIST, JsonLexerKt.END_LIST));
                sb.append("]");
    ...
    private void findElements() {
        if (this.tq.matchChomp("#")) {
            ...
            byAttribute();
        } else if (this.tq.matchChomp(ProxyConfig.MATCH_ALL_SCHEMES)) {
            allElements();
    ...
    private void byTag() {
        ...
        if (consumeElementSelector.contains("|")) {
            consumeElementSelector = consumeElementSelector.replace("|", CertificateUtil.DELIMITER);
        }
        this.evals.add(new Evaluator.Tag(consumeElementSelector.trim().toLowerCase()));
    }

    private void byAttribute() {
        TokenQueue tokenQueue = new TokenQueue(this.tq.chompBalanced(JsonLexerKt.BEGIN_LIST, JsonLexerKt.END_LIST));
