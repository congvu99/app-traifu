...
enum TokeniserState {
    ...
    AttributeValue_unquoted {
        /* access modifiers changed from: package-private */
        public void read(Tokeniser tokeniser, CharacterReader characterReader) {
            String consumeToAny = characterReader.consumeToAny(9, 10, TokenParser.CR, 12, TokenParser.SP, Typography.amp, Typography.greater, 0, '\"', '\'', Typography.less, '=', '`');
    ...
    AfterDoctypeName {
        ...
        public void read(Tokeniser tokeniser, CharacterReader characterReader) {
            if (characterReader.isEmpty()) {
                ...
                tokeniser.emitDoctypePending();
                tokeniser.transition(Data);
            } else if (characterReader.matchesAny(9, 10, TokenParser.CR, 12, TokenParser.SP)) {
                characterReader.advance();
            } else if (characterReader.matches((char) Typography.greater)) {
                tokeniser.emitDoctypePending();
                tokeniser.advanceTransition(Data);
    ...
    static {
        ...
        attributeDoubleValueCharsSorted = new char[]{'\"', Typography.amp, 0};
        attributeNameCharsSorted = new char[]{9, 10, TokenParser.CR, 12, TokenParser.SP, '/', '=', Typography.greater, 0, '\"', '\'', Typography.less};
