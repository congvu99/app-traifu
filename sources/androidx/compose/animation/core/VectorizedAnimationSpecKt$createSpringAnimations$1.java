...
public final class VectorizedAnimationSpecKt$createSpringAnimations$1 implements Animations {
    ...
    VectorizedAnimationSpecKt$createSpringAnimations$1(V v, float f, float f2) {
        ...
        this.$dampingRatio = f;
        this.$stiffness = f2;
        Iterable until = RangesKt.until(0, this.$visibilityThreshold.getSize$animation_core_release());
        float f3 = this.$dampingRatio;
        float f4 = this.$stiffness;
        V v2 = this.$visibilityThreshold;
        Collection arrayList = new ArrayList(CollectionsKt.collectionSizeOrDefault(until, 10));
        Iterator it = until.iterator();
        while (it.hasNext()) {
            arrayList.add(new FloatSpringSpec(f3, f4, v2.get$animation_core_release(((IntIterator) it).nextInt())));
        }
