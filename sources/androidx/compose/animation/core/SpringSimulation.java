...
public final class SpringSimulation {
    ...
    public final long m127updateValuesIJZedt4$animation_core_release(float f, float f2, long j) {
        double d;
        double d2;
        float f3 = f2;
        init();
        ...
        if (f5 > 1.0f) {
            ...
            if (f5 == 1.0f) {
                ...
                double d20 = (double) f4;
                double d21 = d18 * ((((double) f6) * d19 * d20) + ((double) f3));
                double exp4 = Math.exp(((double) (-f6)) * d19 * d3) * ((Math.cos(this.dampedFreq * d3) * d20) + (Math.sin(this.dampedFreq * d3) * d21));
                double d22 = this.naturalFreq;
                float f7 = this.dampingRatio;
                double d23 = (-d22) * exp4 * ((double) f7);
                double exp5 = Math.exp(((double) (-f7)) * d22 * d3);
                double d24 = this.dampedFreq;
                double d25 = exp4;
                double sin = (-d24) * d20 * Math.sin(d24 * d3);
                double d26 = this.dampedFreq;
                d = d23 + (exp5 * (sin + (d21 * d26 * Math.cos(d26 * d3))));
                d2 = d25;
            }
        }
