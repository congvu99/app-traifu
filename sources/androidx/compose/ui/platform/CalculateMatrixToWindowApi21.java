...
final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    private final float[] tmpMatrix = Matrix.m2133constructorimpl$default((float[]) null, 1, (DefaultConstructorMarker) null);

    /* renamed from: calculateMatrixToWindow-EL8BTi8  reason: not valid java name */
    public void m4002calculateMatrixToWindowEL8BTi8(View view, float[] fArr) {
        Intrinsics.checkNotNullParameter(view, ViewHierarchyConstants.VIEW_KEY);
        Intrinsics.checkNotNullParameter(fArr, "matrix");
        Matrix.m2142resetimpl(fArr);
        m4001transformMatrixToWindowEL8BTi8(view, fArr);
    }

    /* renamed from: transformMatrixToWindow-EL8BTi8  reason: not valid java name */
    private final void m4001transformMatrixToWindowEL8BTi8(View view, float[] fArr) {
        ViewParent parent = view.getParent();
        if (parent instanceof View) {
            m4001transformMatrixToWindowEL8BTi8((View) parent, fArr);
            m4000preTranslate3XD1CNM(fArr, -((float) view.getScrollX()), -((float) view.getScrollY()));
            m4000preTranslate3XD1CNM(fArr, (float) view.getLeft(), (float) view.getTop());
        } else {
            int[] iArr = this.tmpLocation;
            view.getLocationInWindow(iArr);
            m4000preTranslate3XD1CNM(fArr, -((float) view.getScrollX()), -((float) view.getScrollY()));
            m4000preTranslate3XD1CNM(fArr, (float) iArr[0], (float) iArr[1]);
        }
        android.graphics.Matrix matrix = view.getMatrix();
        if (!matrix.isIdentity()) {
            Intrinsics.checkNotNullExpressionValue(matrix, "viewMatrix");
            m3999preConcattUYjHk(fArr, matrix);
        }
    }

    /* renamed from: preConcat-tU-YjHk  reason: not valid java name */
    private final void m3999preConcattUYjHk(float[] fArr, android.graphics.Matrix matrix) {
        AndroidMatrixConversions_androidKt.m1796setFromtUYjHk(this.tmpMatrix, matrix);
        AndroidComposeView_androidKt.m3993preTransformJiSxe2E(fArr, this.tmpMatrix);
    }

    /* renamed from: preTranslate-3XD1CNM  reason: not valid java name */
    private final void m4000preTranslate3XD1CNM(float[] fArr, float f, float f2) {
        Matrix.m2142resetimpl(this.tmpMatrix);
        Matrix.m2153translateimpl$default(this.tmpMatrix, f, f2, 0.0f, 4, (Object) null);
        AndroidComposeView_androidKt.m3993preTransformJiSxe2E(fArr, this.tmpMatrix);
    }
}
