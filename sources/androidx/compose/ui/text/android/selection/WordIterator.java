...
public final class WordIterator {
    ...
    public WordIterator(CharSequence charSequence2, int i, int i2, Locale locale) {
        ...
        if (i >= 0 && i <= charSequence2.length()) {
            if ((i2 < 0 || i2 > this.charSequence.length()) ? false : z) {
                BreakIterator wordInstance = BreakIterator.getWordInstance(locale);
                Intrinsics.checkNotNullExpressionValue(wordInstance, "getWordInstance(locale)");
                this.iterator = wordInstance;
                this.start = Math.max(0, i - 50);
                this.end = Math.min(this.charSequence.length(), i2 + 50);
                this.iterator.setText(new CharSequenceCharacterIterator(this.charSequence, i, i2));
                return;
            }
    ...
    private final void checkOffsetIsValid(int i) {
        ...
        if (!z) {
            throw new IllegalArgumentException(("Invalid offset: " + i + ". Valid range is [" + this.start + " , " + this.end + JsonLexerKt.END_LIST).toString());
        }
    }
