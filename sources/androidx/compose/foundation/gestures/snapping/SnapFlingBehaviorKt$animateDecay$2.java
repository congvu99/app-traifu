...
final class SnapFlingBehaviorKt$animateDecay$2 extends Lambda implements Function1<AnimationScope<Float, AnimationVector1D>, Unit> {
    ...
    SnapFlingBehaviorKt$animateDecay$2(float f, Ref.FloatRef floatRef, ScrollScope scrollScope) {
        super(1);
        this.$targetOffset = f;
    ...
    public final void invoke(AnimationScope<Float, AnimationVector1D> animationScope) {
        ...
        if (Math.abs(animationScope.getValue().floatValue()) >= Math.abs(this.$targetOffset)) {
            SnapFlingBehaviorKt.animateDecay$consumeDelta(animationScope, this.$this_animateDecay, SnapFlingBehaviorKt.coerceToTarget(animationScope.getValue().floatValue(), this.$targetOffset) - this.$previousValue.element);
            animationScope.cancelAnimation();
            return;
        }
        SnapFlingBehaviorKt.animateDecay$consumeDelta(animationScope, this.$this_animateDecay, animationScope.getValue().floatValue() - this.$previousValue.element);
