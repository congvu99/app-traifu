...
final class FillModifier extends InspectorValueInfo implements LayoutModifier {
    ...
    public MeasureResult m476measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        int i;
        int i2;
        int i3;
        int i4;
        ...
        if (!Constraints.m4620getHasBoundedWidthimpl(j) || this.direction == Direction.Vertical) {
            i2 = Constraints.m4626getMinWidthimpl(j);
            i = Constraints.m4624getMaxWidthimpl(j);
        } else {
            i2 = RangesKt.coerceIn(MathKt.roundToInt(((float) Constraints.m4624getMaxWidthimpl(j)) * this.fraction), Constraints.m4626getMinWidthimpl(j), Constraints.m4624getMaxWidthimpl(j));
            i = i2;
        }
        if (!Constraints.m4619getHasBoundedHeightimpl(j) || this.direction == Direction.Horizontal) {
            int r2 = Constraints.m4625getMinHeightimpl(j);
            i3 = Constraints.m4623getMaxHeightimpl(j);
            i4 = r2;
        } else {
            i4 = RangesKt.coerceIn(MathKt.roundToInt(((float) Constraints.m4623getMaxHeightimpl(j)) * this.fraction), Constraints.m4625getMinHeightimpl(j), Constraints.m4623getMaxHeightimpl(j));
            i3 = i4;
        }
        Placeable r9 = measurable.m3649measureBRTryo0(ConstraintsKt.Constraints(i2, i, i4, i3));
        return MeasureScope.CC.layout$default(measureScope, r9.getWidth(), r9.getHeight(), (Map) null, new FillModifier$measure$1(r9), 4, (Object) null);
    }
