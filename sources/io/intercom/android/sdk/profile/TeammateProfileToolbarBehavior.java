...
class TeammateProfileToolbarBehavior extends ProfileToolbarBehavior {
    ...
    private void updateTitle(ViewGroup viewGroup, View view, float f) {
        ...
        float max = (float) ((int) Math.max(0.0f, (this.translate28dp / 100.0f) * f));
        if (max > 0.0f) {
            this.subtitle.setTranslationY(max);
        }
        float max2 = (float) ((int) Math.max(0.0f, (this.translate28dp / 100.0f) * f));
        if (max2 > 0.0f) {
    ...
    private void updateAvatar(ViewGroup viewGroup, View view, float f) {
        ...
        this.avatar1.setTranslationX((float) ((int) Math.max(f2, 0.0f)));
        this.activeStateCircle.setTranslationX((float) ((int) Math.max(f2 * (((1.0f - dimensionPixelSize) * 1.4166666f) + 1.0f), 0.0f)));
        float f3 = (this.profileTranslationY / 100.0f) * f;
        this.avatar1.setTranslationY((float) ((int) Math.max(0.0f, f3)));
        this.activeStateCircle.setTranslationY((float) ((int) Math.max(0.0f, f3 * ((dimensionPixelSize * 1.4166666f) + 1.0f))));
    ...
    private void translateY(View view, View view2, float f) {
        view.setTranslationY((float) ((int) Math.max(0.0f, (f / 100.0f) * getScrollPercentage(view2))));
    }
}
